...
const IrregularTetrahedron = (props: MeshProps) => {
    ...
    const normalsForBuffer = new Float32Array(flattenVector3Array(computeNormals(vertices)));
...
export function computeFullVertexArray(vertices:Vector3[],indicesOfFaces:number[]):Vector3[] {
...
export function computeNormals(fullVertices: Vector3[]):Vector3[]{
    if(fullVertices.length % 3 != 0){
        throw new Error(`Faces must be triangles to compute normals. A length of ${fullVertices.length} does not divide into triangles.`)
    }
    const normalsArray:Vector3[] = [];
    for(let i=0; i < fullVertices.length; i+=3){
        const faceNormal=normalFromVertices(fullVertices[i],fullVertices[i+1],fullVertices[i+2]);
...
export function normalFromVertices(a:Vector3,b:Vector3,c:Vector3, normalize=false):Vector3 {
    // addScaledVector changes the inputs. Use clone to prevent this.
    const normal = a.clone().addScaledVector(b,-1).cross(c.clone().addScaledVector(b,-1));
    if(normalize){
